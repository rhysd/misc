(function () {
    ...
    async function main(): Promise<void> {
        ...
        const torusObject = createObject(prog, torus(64, 64, 0.1, 0.4));
        const sphereObject = createObject(prog, sphere(64, 64, 0.25));
        ...
        const lightDirection: [number, number, number] = [-0.577, 0.577, 0.577];
        const cameraPos: [number, number, number] = [0, 5, 5];
        const cameraUp: [number, number, number] = [0, 1, -1];
        ...
        const uniforms = ['mMat', 'vpMat', 'invMat', 'lightDirection', 'eyePosition'].reduce(
            (acc, name) => {
        ...
        function update(): void {
            ...
            q.toVecIII([0, 5, 5], qCamera, cameraPos);
            q.toVecIII([0, 1, -1], qCamera, cameraUp);
