...
vec4 sobelFilter() {
    ...
    for (int j = 0; j <= 2; j++) {
        for (int i = 0; i <= 2; i++) {
            float x = float(i - 1);
            float y = float(j - 1);
            vec2 pos = (origin + vec2(x, y)) * tFrag;
            ...
            horizontal += texture2D(texture, pos).rgb * sobelHorizontalKernel[j * 3 + i];
            vertical += texture2D(texture, pos).rgb * sobelVerticalKernel[j * 3 + i];
        }
    }
